...
static const unsigned char *zi, *trans, *tindex, *types, *abbrevs, *abbrevs_end;
...
static void do_tzset()
{
	...
	if (map) {
		...
		tindex = trans + (zi_read32(trans-12) << scale);
		types = tindex + zi_read32(trans-12);
...
static size_t scan_trans(long long t, int local, size_t *alt)
{
	...
	size_t a = 0, n = (tindex-trans)>>scale, m;
	...
	while (n > 1) {
		...
		if (local) off = (int32_t)zi_read32(types + 6 * tindex[m-1]);
		if (t - off < (int64_t)x) {
			n /= 2;
		} else {
			a = m;
	...
	/* First and last entry are special. First means to use lowest-tindex
	 * non-DST type. Last means to apply POSIX-style rule if available. */
	n = (tindex-trans)>>scale;
	if (a == n-1) return -1;
	if (a == 0) {
		x = zi_read32(trans + (a<<scale));
		if (scale == 3) x = x<<32 | zi_read32(trans + (a<<scale) + 4);
		else x = (int32_t)x;
		if (local) off = (int32_t)zi_read32(types + 6 * tindex[a-1]);
		if (t - off < (int64_t)x) {
	...
	if (alt) {
		if (a && types[6*tindex[a-1]+4] != types[6*tindex[a]+4])
			*alt = tindex[a-1];
		else if (a+1<n && types[6*tindex[a+1]+4] != types[6*tindex[a]+4])
			*alt = tindex[a+1];
		else
			*alt = tindex[a];
	}
